...
\textbf{Idea.} I use the distributed formation control law from Algorithm 2 of the MAS Algorithms lecture. Each agent has single integrator dynamics $\dot p_i = u_i$, and the discrete time update with step $\Delta t$ is
\begin{align*}
...
\textbf{Observation.} Even though we ask for $7$ back to back reshapes of the swarm, every letter comes out recognisable at the end of its phase. Agents that were on the wrong side of the previous letter's shape cross over during the transition because the update only cares about the relative offset $r_j - r_i$, not the absolute location.
...
\textbf{Choice of $W$.} The lecture asks that $W$ is doubly stochastic and $W_{ij} = 0$ for $j$ not a neighbour. I use the Metropolis-Hastings weights,
\begin{align*}
    W_{ij} \;=\; \frac{1}{1 + \max(\deg(i), \deg(j))}, \qquad (i,j) \in E, \quad i \ne j,
\end{align*}
and $W_{ii} = 1 - \sum_{j \ne i} W_{ij}$. This satisfies all three required properties.
...
\textbf{Numerical readouts.} Final $\|e(T)\| \approx 0.17$, mean $\|e(t)\| \approx 0.27$, and the maximum consensus gap across time is $\approx 0.26$ so the agents almost fully agree at every $t$. The intruder drifts about $2.7$ units over $T_{\max}$ steps, so the estimator tracks it with roughly $10\%$ relative error. The lecture warns on page 8 that constant step DGD has an $O(\alpha)$ steady state error, this is exactly that.
...
\textbf{Step 2: LP relaxation.} The lecture hints that we should do a convex relaxation. The usual trick is
\begin{align*}
...
\textbf{Step 4: scaled form ADMM updates.} With scaled dual $U^{(i)}$, penalty $\rho > 0$, and Frobenius norm, the scaled form ADMM from the Helper notes and the ADMM lecture gives:
\begin{align*}
...
\begin{itemize}[nosep]
    ...
    \item The primal residual decays monotonically and the objective settles to the LP optimum, which means the algorithm is doing what it should.
\end{itemize}
...
\textbf{Step 2: augmented Lagrangian.} With penalty $\rho > 0$ and dual $y$,
\begin{align*}
